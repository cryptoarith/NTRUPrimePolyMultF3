...
/*************************************************
* Name:         A3-Hybrid2 Algorithm corresponds to a3_real() function below. 
...
void ka2_real(struct complex a[], struct complex b[], struct complex c[], int n){
	...
	/* base case */
	if(n == 32){   /* A3-Hybrid2 is derived from A3-Hybrid by modifying this base value from n=4 to n=32 */
...
void ka2_complex(struct complex a[], struct complex b[], struct complex c[], int n){
	...
	#define KA2_COMPLEX_MAX_AB_SIZE 8 // n/2
	#define KA2_COMPLEX_MAX_P_SIZE 15 // n-1
...
void n1_complex(struct complex a[], struct complex b[], struct complex c[], int n){
	...
	#define N1_COMPLEX_MAX_AB_SIZE 64 // n/4
	#define N1_COMPLEX_MAX_P_SIZE 127 // 2*n/4-1

	/* base case */
	if(n == 16){
		sb_comba_complex(a, b, c, n); /* A3-Hybrid2 is derived from A3-Hybrid by modifying this base value from n=4 to n=16 */
		return;
	}
...
void n1_real(struct complex a[], struct complex b[], struct complex c[], int n){
	...
	#define N1_REAL_MAX_AB_SIZE 64 // n/4
	#define N1_REAL_MAX_P_SIZE 127 // 2*n/4-1
	...
	struct complex u6[N1_REAL_MAX_P_SIZE];
	struct complex u7[N1_REAL_MAX_P_SIZE];
	struct complex u8[N1_REAL_MAX_P_SIZE];
	struct complex u9[N1_REAL_MAX_P_SIZE];
...
/* The A3-Hybrid2 Algorithm corresponds to a3_real() function below. See the paper: https://eprint.iacr.org/2020/1336.pdf  for the details*/

void a3_real(struct complex a[], struct complex b[], struct complex c[], int n){

	#define A3_REAL_MAX_AB_SIZE 256 // n/3
	#define A3_REAL_MAX_P_SIZE 511 // 2*n/3-1
	...
	struct complex ra1[A3_REAL_MAX_AB_SIZE];
	struct complex ra2[A3_REAL_MAX_AB_SIZE];
	struct complex ra3[A3_REAL_MAX_AB_SIZE];
	struct complex ra4[A3_REAL_MAX_AB_SIZE];
	struct complex ra5[A3_REAL_MAX_AB_SIZE];
	struct complex rb1[A3_REAL_MAX_AB_SIZE];
	struct complex rb2[A3_REAL_MAX_AB_SIZE];
	struct complex rb3[A3_REAL_MAX_AB_SIZE];
	struct complex rb4[A3_REAL_MAX_AB_SIZE];
	struct complex rb5[A3_REAL_MAX_AB_SIZE];
	...
	struct complex p0[A3_REAL_MAX_P_SIZE];
	struct complex p1[A3_REAL_MAX_P_SIZE];
	struct complex p2[A3_REAL_MAX_P_SIZE];
	struct complex p3[A3_REAL_MAX_P_SIZE];
	struct complex p4[A3_REAL_MAX_P_SIZE];
	memset(p0, 0, sizeof(p0));
	memset(p1, 0, sizeof(p1));
	memset(p2, 0, sizeof(p2));
	memset(p3, 0, sizeof(p3));
	...
	n1_real(a0, b0, p0, n3);
	n1_real(ra3, rb3, p1, n3);
	n1_complex(ra4, rb4, p2, n3);
	phase_change(p2, p3, p_size);
	n1_real(a2, b2, p4, n3);

	struct complex u1[A3_REAL_MAX_P_SIZE];
	struct complex u2[A3_REAL_MAX_P_SIZE];
	struct complex u3[A3_REAL_MAX_P_SIZE];

	sum_real(p0, p4, u1, p_size);
	sum_real(p1, p2, u2, p_size);
	sum_neg_neg_real(u1, u2, u3, p_size);

	struct complex c1[A3_REAL_MAX_P_SIZE];
	struct complex c2[A3_REAL_MAX_P_SIZE];
	struct complex c3[A3_REAL_MAX_P_SIZE];

	struct complex *c0 = p0;

	sum_re_neg_im_real(u3, p2, c1, p_size);
	difference_real(u1, p2, c2, p_size);
	sum_re_im_real(u3, p2, c3, p_size);

	struct complex *c4 = p4;

	sum_real(c, c0, c, p_size);
	sum_real(c+n3, c1, c+n3, p_size);
	sum_real(c+2*n3, c2, c+2*n3, p_size);
	sum_real(c+3*n3, c3, c+3*n3, p_size);
	sum_real(c+4*n3, c4, c+4*n3, p_size);
...
int main(){
    ...
    fclose(myFile);
        ...
        /*write on c_a3_hybrid2 file*/	
	FILE *outFile;
    outFile = fopen("c_a3_hybrid2", "w");
    if (outFile == NULL)
	{
